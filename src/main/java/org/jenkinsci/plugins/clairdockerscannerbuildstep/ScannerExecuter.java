...
public class ScannerExecuter {
	...
	public static int execute(AbstractBuild build, Launcher launcher, BuildListener listener, String artifactName,
		...
		PrintStream History_print_stream = null;
		// try {
		...
		ArgumentListBuilder args1 = new ArgumentListBuilder();
		String Docker="docker";
		String History="history";
			
		args1.add(Docker,History,localImage);
		File HistoryoutFile = new File(build.getRootDir(), "Historyout");
		//File HistoryoutFile1 = new File(build.getRootDir(), "Historyout1");
		Launcher.ProcStarter ps1 = launcher.launch();
		ps1.cmds(args1);
		ps1.stdin(null);
		ps1.stderr(listener.getLogger());
		History_print_stream = new PrintStream(HistoryoutFile, "UTF-8");
		ps1.stdout(History_print_stream);
		exitCode = ps1.join();
		/*FilePath workspace1 = build.getWorkspace();
		FilePath target1 = new FilePath(workspace1, artifactName);*/
		FilePath HistoryoutFilePath = new FilePath(HistoryoutFile);

		System.out.println("File path of history file"+HistoryoutFilePath);
		//System.out.println("Build directory"+build.getRootDir());
		...
		FilePath filename = Text_HTMLConverter.text_to_html(outFile, outfilFilePath1, out, build_no, target, build,
				jenkins_home,listener,buildNo,HistoryoutFile,localImage);
